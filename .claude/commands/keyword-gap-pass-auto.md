...
3. **`SEMRUSH_API_KEY` is set** (env, or the routine prompt passes it inline). The manual skill STOPs and asks the user here — the auto variant instead **skips the run and emails failure** ("SEMRUSH_API_KEY not provisioned"). **Never** hardcode a key into a command file and **never** invent gap data to work around a missing key.
