...
| SEMRUSH returned no data for **every** row (quota exhausted or API error) and the DataForSEO gap engine (`scripts/lib/dfs_keyword_gap.py`) also returned nothing | skip remaining phases | `failure` (names the API error) |
