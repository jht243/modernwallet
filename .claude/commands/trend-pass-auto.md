...
6. **SEMrush key** best-effort (NOT a gate): `$SEMRUSH_API_KEY` (from `~/.claude/secrets.env`). Absent → the Phase 3 SEMrush step self-skips (autocomplete still runs); note "SEMrush: skipped — no key" in the digest. Never block the run on this.
