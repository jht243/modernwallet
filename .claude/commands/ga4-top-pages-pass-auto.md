...
2. **Branch is committable (NOT required to be `main`).** Cloud routines run on an ephemeral `claude/*` branch — expected; commit + push **that** branch (`git push origin HEAD`). Only abort on detached HEAD with no branch name.
...
> **Git model (cloud-safe).** Commit on the **current branch**, push `git push origin HEAD`. MUST NOT create a branch or `git checkout main`. Because the GSC trend pass committed to main 2h earlier, `git fetch origin main && git rebase origin/main` before pushing if behind.
