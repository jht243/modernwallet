...
- ✗ "your current system" · "before you switch" · "as you know" · "leaving" the product as if they already use it
- ✓ "you can compare them on price" · "If you already use this product…"
...
**Never duplicate it.** Do not repeat the tee-up, verbatim or near-verbatim, in the subtitle or any other header field. A reader who sees the same sentence twice in a row, once as a deck and once in the body, is looking at a copy-paste error, not a voice.
...
- **GATE — Register.** The row must carry `register: operator` or `register: reporter`, and it must be right for what the page does. Missing record → FAIL. Wrong register → FAIL. Then check the page against it, **both directions**:
  - operator page written in detached third-person encyclopedia prose → FAIL.
