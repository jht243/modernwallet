...
**LOAD RECEIPT — print this, every run.** The moment you finish loading the three files, print one line in your phase output:

`standard-loaded: cs=<first 8 chars of this file's sha256> aa=<same for _anti-ai-language.md> exp=<same for _experience.md> domain="<first 6 words of ## DOMAIN>"`

Get the hashes with a real command (`shasum -a 256 .claude/commands/_content-standard.md | cut -c1-8`), never from memory or a guess. This line is the only evidence the files were actually read rather than assumed, so a run without it is treated as a run that never loaded the standard. The audit phase checks for it and fails the run if it is missing, malformed, or carries a hash that does not match the file on disk.
