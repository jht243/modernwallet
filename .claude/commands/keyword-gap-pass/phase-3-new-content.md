...
1. **The client roster** (see `seo-content-guidelines` memory / the roster the skill loads): Multiple HOA & condo boards across the USA, The HOA Guide (thehoaguide.com), TheBotScout (humanoid-robotics research), DroneAndDefense (defense/drones investment journal) — plus **approved, resume-backed first-hand experience**: Amazon (consumer-electronics email/SEO/paid search at $200M+ scale), Uber (product/pricing/growth), Horizen Labs (growth marketing & retention), and the founder track record (Layer 3 Labs — 1.5M users with no ad spend, 17+ games, two prior exits). Also approved: PBA (Professional Bowlers Association); MetaKing Studios & HeroMaker Studios.
...
**If no client and no routine honestly fits** (true for most ModernWallet personal-finance calculator content — none of the client roster or the SEO-routine portfolio has topical overlap with mortgage/auto/investing math), anchor the page another way: an original number/benchmark you actually produced, an original artifact, a non-obvious tradeoff, or a defensible POV with reasoning. **For this project specifically, a hand-verified worked numeric example (computed step-by-step with the same formulas the page's calculator uses) satisfies this bar** — it is a first-hand computed artifact a generic LLM would not reliably reproduce correctly, matching the existing spoke pages' pattern (see `src/data/spokes-*.ts`). Only if none of that honestly applies may the page ship without an anchor — and you must leave a one-line note for the auditor: `anchor-exempt: [why no client, no routine, and no first-hand angle honestly fit this topic]`. The default is to include an anchor; the exemption bar is high.
