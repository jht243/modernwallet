...
2. **Branch is committable; deploy target is `main`** — cloud routines run on an ephemeral `claude/*` branch; that is expected. Commit on the current branch, then deploy to `main` via `git fetch origin main && git rebase origin/main && git push origin HEAD:main`. Only abort on detached HEAD. Never `git checkout main` or create a new branch. Never abort merely because the branch isn't `main`.
