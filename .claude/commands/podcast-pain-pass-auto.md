...
- Keyword demand via `scripts/lib/keyword_data.py` — the ladder picks the best usable provider per field (volume: ahrefs > dataforseo > semrush; kd: ahrefs > semrush > dataforseo), then public-source estimate. **No key is a precondition and the run NEVER aborts for a dry key**; it demotes a rung and labels estimated rows. See `.claude/commands/_keyword-demand-ladder.md`.
