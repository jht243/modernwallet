...
**Format: infra work (populate data), not new page code.** The route and template already exist and are proven (see `offer-in-compromise-calculator` and `estate-planning`'s will-cost-calculator, both of which already have 50-state `stateVariants` + partial `STATE_OVERRIDES` batches in `src/data/state-overrides.ts`; the probate spokes also have 50-state `stateVariants` set but currently have zero entries in `STATE_OVERRIDES` — they render on templated defaults only, illustrating the shell-works-immediately fallback behavior described below rather than a bespoke-content example). This is purely a content-population task: add `stateVariants` to the `retirement-savings-calculator` spoke entry, then author a `RETIREMENT_SAVINGS_STATE_OVERRIDES` batch keyed by state slug.
