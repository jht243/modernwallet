# ROW PROMPT — advanced-estate-planning-strategies--irs-scrutiny (BODY ENRICHMENT, not a new page)

## TASK (one line)
Write **one or two paragraphs of plain prose — NO heading** — to be APPENDED to the end of the existing "Grantor Retained Annuity Trust (GRAT)" section of `/guides/advanced-estate-planning-strategies/`. The addition covers IRS scrutiny of GRATs in 2026: the WSJ-reported Elcan/HCA U.S. Tax Court dispute (pending and disputed), the operational rules that dispute turns on, and a pointer to the site's dedicated GRAT guide.

## WHY (the signal)
The 21 September 2026 Wall Street Journal report on the IRS challenging GRATs is driving demand to this page, and its GRAT section says nothing about enforcement risk. This is chart row 4 of the mindmap-pass 2026-09-21 run.

## HARD CONSTRAINTS
- **ADDITIVE ONLY.** Do not restate, contradict, or rewrite anything already in the page text below. The section already covers the mechanic, the Section 7520 hurdle rate, zeroed-out GRATs, mortality risk, and rolling GRATs — do not repeat them.
- **No heading.** Return prose paragraphs only, separated by a blank line.
- Target length: 120–200 words. Two paragraphs maximum.
- Must include ONE internal link, exactly: `[what a GRAT is and how it works](/guides/what-is-a-grat-trust/)`.
- Must NOT add any external link. The Wall Street Journal is named in prose only; it must not be linked.
- Match the page's existing voice: plain, declarative, second person for the reader, no hype.

## WHAT TO COVER
1. The IRS is challenging how some GRATs were operated. Per the WSJ (21 September 2026), it is asserting a $736 million gift tax bill against a Nashville couple, Chuck and Trisha Elcan, over GRATs that held HCA Healthcare shares. They are disputing it in U.S. Tax Court and the case is pending and unresolved.
2. **You may NOT write that the IRS won, that a court has ruled, that GRATs are illegal or disallowed, or that the law has changed.** The only accurate framing is: the IRS is challenging how these particular GRATs were operated, and the taxpayers dispute it.
3. What that means practically for anyone using a GRAT: the annuity must actually be paid at least annually, hard-to-value assets need defensible independent valuations, and the paperwork has to hold up. These are the operational points a challenge turns on.
4. Close with the pointer to the dedicated guide.
