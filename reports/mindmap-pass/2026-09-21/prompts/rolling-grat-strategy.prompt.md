# ROW PROMPT — rolling-grat-strategy (guide page, ModernWallet / themodernwallet.com)

## PAGE
- route: /guides/rolling-grat-strategy/
- slug: rolling-grat-strategy
...
## KEYWORDS + INTENT
- primary keyword: rolling grat
- secondary keywords (DataForSEO volume, US): rolling grat (140/mo, $25.68 CPC); zeroed out grat (70); rolling grat strategy (10); zeroed out grat example (10); grat annuity payment calculation (10); rolling grat example; rolling grat calculator
- intent: Strategy/how-to intent from a reader who already knows what a GRAT is and wants to understand the rolling, short-term, re-funding version the news story describes.

## SERP EVIDENCE (live read, DataForSEO)
Live SERP (DataForSEO, 2026-09-21): verdict winnable. Page one = bernstein.com, mai.capital, wyotrustattorney.com, mayalaw.com, learn.valur.com, fidelity.com, youtube.com, ejrosenlaw.com, silvermanschermer.com, btcpa.net - boutique advisors and law firms, only 2/10 major platforms. AI Overview present, citing bernstein.com, mai.capital, ejrosenlaw.com, learn.valur.com. Related searches: rolling grat example; rolling grat calculator; rolling GRAT strategy; zeroed-out GRAT. Recommended shape: standard article/guide is viable.

## FAQ SPEC — use these REAL People-Also-Ask questions, verbatim as the `faqs[].question` values
  1. What does GRAT mean?
  2. How long can a GRAT last?
  3. How much does a GRAT cost?
  4. What is the difference between a SLAT and a GRAT?
  5. Who pays taxes on a GRAT?
  6. Why would someone use a GRAT?
...
1. What a rolling GRAT is, in one extractable opening sentence (a series of short GRATs, each annuity payment re-funding the next).
2. The zeroed-out math: annuity set so the remainder's actuarial value is near zero, so the taxable gift at funding is near $0. Explain in words + ONE labelled hypothetical illustration using the September 2026 rate of 5.40% with the month named.
3. Why two-year terms rather than one ten-year term (mortality exposure per trust; a losing period wastes one GRAT, not the plan).
4. What happens in a losing year - assets come back through the annuity, balance of zero, nothing transferred, cost is fees and time. Use the Kaercher "largely no-regrets planning" quote and the Wagner "winning years and losing years" quote, both attributed to the WSJ report.
5. What the strategy has historically produced - the Laski/GRATtrack $1M Amazon -> ~$4.5M estimate, clearly labelled as one practitioner's estimate reported by the WSJ and NOT a guaranteed or typical result, with the point that it depends entirely on the asset outperforming the hurdle rate.
6. The operational discipline the IRS is now focused on: annuity paid at least annually, valuation of hard-to-value assets, documentation - framed through the pending, disputed Elcan/HCA Tax Court case per the fact list.
7. When rolling GRATs fit (concentrated, volatile, appreciating positions) and when they do not (illiquid assets, short life expectancy, small estates well under the $15M exemption).
8. What it costs to run a rolling programme (each GRAT is its own trust with its own drafting and administration; the $2,500-$10,000+ per-trust range as typical).
9. Pointer to /guides/what-is-a-grat-trust/ for the basics and /compare/grat-vs-idgt/ for the long-horizon alternative.
