...
- `h1` (string).
- `cardBlurb` (string) — one line for index cards, about 100 to 140 chars.
- `introText` (string) — 3 short paragraphs, separated by \n\n.
- `sections` (array of 6 to 8 objects) — each `{ "heading": string, "body": string }`. Each body is 2 to 4 paragraphs separated by \n\n.
- `tools` (array of objects) — each `{ "href": string, "label": string }`. Use only calculator routes listed in the row prompt. 2 to 4 entries.
- `faqs` (array of 6 objects) — each `{ "question": string, "answer": string }`. Questions come from the FAQ spec in the row prompt, worded naturally.
- `sources` (array of 2 to 4 objects) — each `{ "label": string, "url": string }`. URLs must come from the allowed list.
