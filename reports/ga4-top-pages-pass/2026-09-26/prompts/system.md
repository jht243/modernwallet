...

{
  ...
  "metaDescription": "VOO vs SPY compared: expense ratio, fund structure, and liquidity, so you can see which S&P 500 ETF fits your investing style.",
  ...
  "introText": "VOO has a 0.03% expense ratio and is structured as an ETF share class of an open-end [Vanguard](https://investor.vanguard.com) mutual fund. SPY, by contrast, charges 0.0945% and is structured as a unit investment trust with the deepest trading volume of any ETF in the world. So the better choice usually depends on whether you're a buy-and-hold investor looking for the lowest cost or an active trader who needs maximum liquidity.",
  "comparisonTable": {
    "rows": [
      {
  ...
  "calculatorLinks": [
    ...
    },
    {
      ...
      "href": "/investing/compound-interest-calculator/"
    }
  ]
}
...
- `updated` — string, always `"2026-09-26"`
- `slug` — string, exactly the slug given in the row prompt
- `title` — string, ≤ 60 characters, ALL entities named
...
- `h1` — string
- `introText` — string. AEO: the first sentence must state the actual difference between the
  entities completely on its own.
- `comparisonTable` — `{ "rows": [ { "dimension": string, "a": string, "b": string } ] }`
- `verdict` — string. Who should pick which option, and the condition that decides it.
...
- OBJECTIVITY: no option is the automatic winner. Every `dimension` row must state a real
  trade-off, and at least one row must favour each option named. The verdict names the condition
  that decides it, not a blanket recommendation.
- `a` and `b` cells are short phrases, not sentences with periods.
- YMYL finance: no individualized investment, tax, or retirement advice. Describe rules and
  tradeoffs; point the reader to a licensed financial advisor, CPA, or attorney for their own
  situation.
- This page compares THREE funds (VOO, SPY, QQQ), not two — the `comparisonTable.rows` "b" column
  and prose must clearly cover QQQ's genuinely different index (Nasdaq-100, growth/tech-concentrated)
  alongside the existing VOO/SPY (S&P 500) pairing already covered on the sibling `/compare/voo-vs-spy/`
  page. Do not just repeat that page's S&P-500-only framing — the reason this page exists is QQQ's
  different index composition, concentration, and volatility profile.
