...
import { computePersonalLoanPayoff, type PersonalLoanPayoffInput } from "../lib/personal-loan-payoff";
import { fmtUSD, fmtMonths } from "../lib/format";

// Personal-loan extra-payment / payoff calculator island. For a standard amortizing unsecured
// personal loan you already have (debt consolidation, medical bills, home improvement) — NOT the
// origination-fee/effective-APR math in PersonalLoanCalculator.tsx, which prices a NEW loan.
// Standalone — no host coupling (mirrors BusinessLoanPayoffCalculator / AutoLoanCalculator).
...
export default function PersonalLoanPayoffCalculator({ initialData, heading, subheading }: Props) {
  const [goal, setGoal] = useState<Goal>("extra-payment");
  const [v, setV] = useState<PersonalLoanPayoffInput>({
    loanBalance: n(initialData?.loanBalance, 15000),
    interestRatePct: n(initialData?.interestRatePct, 12.5),
    remainingTermMonths: n(initialData?.remainingTermMonths, 48),
    extraMonthlyPayment: n(initialData?.extraMonthlyPayment, 100),
    targetPayoffMonths: n(initialData?.targetPayoffMonths, 24),
  });
  const effectiveInput: PersonalLoanPayoffInput =
    goal === "target-date" ? { ...v, targetPayoffMonths: v.targetPayoffMonths } : { ...v, targetPayoffMonths: 0 };
  const r = useMemo(() => computePersonalLoanPayoff(effectiveInput), [effectiveInput]);
  const set = (p: Partial<PersonalLoanPayoffInput>) => setV((s) => ({ ...s, ...p }));
      ...
      <div style={S.grid}>
        <div style={S.inputs}>
          ...
          <div style={S.row2}>
            ...
            <Field label="Remaining term (months)"><Plain v={v.remainingTermMonths} max={120} on={(x) => set({ remainingTermMonths: x })} /></Field>
          </div>
          {goal === "extra-payment" ? (
            <Field label="Extra monthly payment"><Money v={v.extraMonthlyPayment} on={(x) => set({ extraMonthlyPayment: x })} /></Field>
          ) : (
            <Field label="Debt-free in how many months?"><Plain v={v.targetPayoffMonths ?? 24} max={v.remainingTermMonths} on={(x) => set({ targetPayoffMonths: x })} /></Field>
          )}
        </div>
        ...
        <div style={S.results}>
          ...
          <div style={S.statRow}>
            <Stat label="Scheduled payment" value={fmtUSD(r.scheduledPayment)} />
          ...
          <div style={S.disclaimer}>
            Estimate only. Assumes a standard fully-amortizing personal loan (fixed rate, extra payment applied straight to principal) and does not include any origination fee — see our <a href="/personal-loan/" style={S.link}>personal loan calculator</a> for the fee-adjusted cost of a new loan. Confirm your lender applies extra payments to principal and does not charge a prepayment penalty before sending extra money; most personal loan lenders don't, but a few still do.
          </div>
        </div>
      </div>
    </div>
