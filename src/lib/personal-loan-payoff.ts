// Pure personal-loan payoff engine.
//
// For a standard amortizing unsecured personal loan (debt consolidation, home improvement, medical
// bills, etc.) — NOT the origination-fee/effective-APR math in personal-loan.ts, which answers a
// different question (what does a NEW loan actually cost). This engine answers a payoff question
// for a loan you already have: how much does an extra monthly payment save, or how large an extra
// payment do you need to be debt-free by a target date.
//
// Formula matches src/lib/business-loan-payoff.ts's amortization math (standard fully-amortizing
// payment, then a month-by-month extra-principal simulation) — the same battle-tested engine
// pattern, kept as an independent copy here so a change to that tool can never silently affect
// this one.

export interface PersonalLoanPayoffInput {
  loanBalance: number;
  ...
  /** Goal mode 2: "I want to be debt-free in N months" — solve for the required extra payment
   *  instead of taking one. When set (>0), this overrides extraMonthlyPayment. */
...
export interface PersonalLoanPayoffResult {
   ...
   *  unless that mode is active, or 0 if the target is already at/beyond the baseline payoff. */
  requiredExtraForTarget: number | null;
}

const EMPTY: PersonalLoanPayoffResult = {
...
function amortize(
  ...
  let totalInterest = 0;
  let month = 0;
  const HARD_CAP_MONTHS = 240; // 20 years — well past any realistic personal loan term
...
export function computePersonalLoanPayoff(input: PersonalLoanPayoffInput): PersonalLoanPayoffResult {
  const principal = Math.max(0, input.loanBalance ?? 0);
  const rate = Math.max(0, input.interestRatePct ?? 0);
  const term = clampInt(input.remainingTermMonths, 1, 120);
